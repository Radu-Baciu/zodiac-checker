...
public class WinterCheckerImpl extends WinterCheckerGrpc.WinterCheckerImplBase {
    @Override
    public void giveInfoWinter (ZodiacChecker.ZodiacRequest request, StreamObserver<ZodiacChecker.ZodiacReply> responseObserver) {
        String[][] zodiac = new String[3][3];
        int zodiacBuilder = 0;
        try {
            Scanner fileReader = new Scanner(new BufferedReader(new FileReader("src/main/java/services/WinterZodiac.txt")));
        ...
        if (date.compareTo(zodiac[0][1]) <= 0)
            sign = zodiac[0][2];
        if (date.compareTo(zodiac[0][0]) >= 0)
            sign = zodiac[0][2];
