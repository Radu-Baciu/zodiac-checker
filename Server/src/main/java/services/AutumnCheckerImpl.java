...
public class AutumnCheckerImpl extends AutumnCheckerGrpc.AutumnCheckerImplBase {
    @Override
    public void giveInfoAutumn (ZodiacChecker.ZodiacRequest request, StreamObserver<ZodiacChecker.ZodiacReply> responseObserver) {
        String[][] zodiac = new String[3][3];
        int zodiacBuilder = 0;
        try {
            Scanner fileReader = new Scanner(new BufferedReader(new FileReader("src/main/java/services/AutumnZodiac.txt")));
